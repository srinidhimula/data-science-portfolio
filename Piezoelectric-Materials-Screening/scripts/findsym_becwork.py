...
def writeCIF(cell, prec, basename,outputdir):
    ...
    filename = (basename + '_' + sgid + '.cif')#.replace('/', '|'), 'w'
    full_path = Path(outputdir) / filename
...
def main_findsym(ciffile,outputdir):
    try:
        #cell = ase.io.read(ciffile)
        ase_cell = ase.io.read(ciffile)
        cell = (ase_cell.cell, ase_cell.get_scaled_positions(), ase_cell.get_atomic_numbers())

    except Exception as e:
        print('Could not read CIF structure from file: ' + ciffile)
        print('Error message is: ' + str(e))
        sys.exit(1)
    basename = ciffile.stem
    ...
    for prec in precs:
        ...
        if s != old:
            #print(f'{prec}\t\t{s}')
            sgnum=writeCIF(cell, prec, basename,outputdir)
            old = s
    return(sgnum)